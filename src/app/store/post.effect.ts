    ...
    getPost$ = createEffect(
      () => {
        return  this.actions$.pipe(
          ...
          mergeMap((_) => this.http.get<any>('http://localhost:3000/api/posts')
            .pipe(
              map((result: any) => {
      ...
      createPost$ = createEffect(
        () => {
          return  this.actions$.pipe(
            ...
            mergeMap(([{post}]) => {
              return this.http.post<fromReducer.State>('http://localhost:3000/api/posts', post)
            .pipe(
        ...
        deletePost$ = createEffect(
          () => {
            return  this.actions$.pipe(
              ...
              withLatestFrom(this.store.pipe(select(selectPosts))),
              mergeMap(([{id}]) => {
                return this.http.delete<fromReducer.State>('http://localhost:3000/api/posts/'+ id)
              .pipe(
